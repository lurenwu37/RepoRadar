...
def main():
    ...
    print("=" * 60)
    print("  Zhoubao Bot — AI News Radar")
    print("=" * 60)

    # 1. Fetch 30 unique links (default days_back=1)
    links = fetch_all_links(days_back=1)

    # 2. Score and generate top-5 recommendations via LLM
    scored_links, recommendations = score_and_recommend(links)

    # 3. Send beautiful card to Feishu (passing days_back=1)
    import datetime
    today = datetime.date.today().strftime("%Y-%m-%d")
    title = f"📡 AI 资讯精选推荐 · {today}"

    send_to_feishu(title, recommendations, total_count=len(links), days_back=1)
