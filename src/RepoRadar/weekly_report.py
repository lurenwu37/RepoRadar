...
def main():
    ...
    print("=" * 60)
    print("  Zhoubao Bot — AI Weekly Report")
    print("=" * 60)

    # 1. Fetch 30 unique links for the past 7 days
    links = fetch_all_links(days_back=7)
    ...
    # Calculate start date of the week (7 days ago)
    start_date = (today - datetime.timedelta(days=7)).strftime("%Y-%m-%d")
    end_date = (today - datetime.timedelta(days=1)).strftime("%Y-%m-%d")
    title = f"📡 GitHub 周度明星雷达 ({start_date} ~ {end_date})"

    send_to_feishu(title, recommendations, total_count=len(links), days_back=7)
